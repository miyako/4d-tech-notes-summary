# Tech Note 01-32: PostImage

**Author:** Not specified in source
**Published:** July 31, 2001 | **Product/Version:** 4D v6.7 | **Platform:** Mac & Win
**Page:** https://kb.4d.com/assetid=16388
**Download:** https://kb.4d.com/ftp://ftp.4D.com/ACI_TECHNICAL_NOTES/2001/Windows/TN_2001_31-35_(JUL)/01-32_PostImage.exe (dead link — see Historical Context)

## Overview

Only a short teaser paragraph for this Tech Note survives in this archive; the full PDF and its companion example database could not be
recovered. The complete text of the surviving teaser is reproduced below:

> This technical note demonstrates a technique for uploading pictures from a Web browser and storing them in the database. That technique can easily be integrated in existing database even by beginners.

## Key Points

Based on the available teaser text, this note is: a technique for uploading pictures from a web browser and storing them in a 4D database.

## Featured Technology

- Web picture upload
- 4D built-in Web Server
- Browser-to-database file transfer

## Historical Context

Published July 2001 for 4D v6.7, this note dates from the "4D 2001"/"4D 2000-2001" product era (the v6.5/v6.7/v6.8 lineage),
running on classic Mac OS 9 (with Mac OS X newly released in 2001) and Windows 98/2000/NT. This was years before Project Mode
(introduced 4D v17, 2018), ORDA (2018+), and 4D's own SQL engine (introduced 4D v11 SQL, ~2007) existed — development happened entirely
in binary Design Mode (.4DB/.4DC structure files) using the classic procedural/set-based 4D language.

The full archive for this note could not be recovered: the linked download was an old Windows self-extracting .exe installer that could
not be extracted in this environment (error_reason: `NO_PDF_IN_ARCHIVE_TEASER_ONLY`), so this page is necessarily based only on the short
teaser paragraph published on the Tech Note's web page, not the full PDF or its companion example database.

**Status: Partially superseded**

This note demonstrates uploading pictures from a web browser and storing them directly in a 4D database, described as an approachable technique even for beginners. The general goal — accepting image uploads from a web client into 4D — remains a common requirement, but the specific 2001-era web-server upload mechanism has been superseded by modern 4D REST/ORDA endpoints and richer built-in support for handling multipart form uploads and BLOB/picture fields.

**What has changed since:**

- 4D's REST/ORDA APIs now provide more modern mechanisms for handling file/image uploads from web and mobile clients
- 4D's built-in Web Server has gained more native multipart/form-upload handling since 2001
